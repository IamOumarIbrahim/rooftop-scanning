...
\input{tables/tab_prior_art.tex}
...
\begin{equation}
w^* = \arg\min_{w_k} \sqrt{(\bar{\phi}_k - \phi_0)^2 + (\bar{\lambda}_k - \lambda_0)^2 \cos^2(\phi_0)}
...
\end{align}
where $R_e = 6{,}371{,}000$~m is the mean volumetric radius of the Earth. For building footprints with spans under 500~m, the geometric scale distortion of this equirectangular projection is bounded by $\mathcal{O}((\Delta \phi)^2) < 0.01\%$ relative to conformal transverse Mercator projections, eliminating the need for heavy geospatial GIS projection dependencies.

\subsection{Stage 4: Gross Footprint Area via Gauss Shoelace Formula}

The gross planar footprint area $A_{\mathrm{raw}}$ of the projected polygon with $n$ ordered boundary vertices $(x_0, y_0), \dots, (x_{n-1}, y_{n-1})$ along a simple non-self-intersecting Jordan curve is computed using the Gauss shoelace formula:
\begin{equation}
...
\end{equation}
By Steiner's polynomial for Minkowski erosion, the exact inward setback area of any planar convex polygon is $A(s) = A_{\mathrm{raw}} - P \cdot s + \pi s^2$. Because the positive corner term $\pi s^2$ is omitted, the linear perimeter formula in \eqref{eq:usable_area} represents a strictly conservative lower bound on usable rooftop area, underestimating true usable area by only $\pi (1.5)^2 \approx 7.07\text{ m}^2$ (under $0.5\%$ on commercial facilities).
...
f_{\mathrm{orient}} &= \max \left( 0.5, \, \cos\left(\frac{\pi \Delta \psi}{360}\right) \cos\left(\frac{\pi |\beta - 20^\circ|}{360}\right) \right)
\end{align}
...
\input{tables/tab_validation_dataset.tex}
\input{tables/tab_error_metrics.tex}
...
\input{tables/tab_w5_case_study.tex}
...
Applying the standard 1.50~m perimeter setback along the 165.40~m perimeter subtracts 248.10~m$^2$, yielding a net usable rooftop area of $1{,}361.92$~m$^2$. At 20\% module efficiency, the recommended DC capacity is $272.38$~kW DC, paired with a recommended $226.99$~kW AC inverter capacity ($\mathrm{ILR} = 1.20$). Accounting for the building's dominant azimuth ($\psi = 68.2^\circ$, orientation derate $f_{\mathrm{orient}} = 0.5601$), racking tilt ($\beta = 15.0^\circ$), regional irradiance (5.50~PSH), and performance losses ($\mathrm{PR} = 0.85$), the system generates an estimated $260.33$~MWh annually. At an electricity tariff of 0.38~AED/kWh, annual utility savings reach 98,925~AED, achieving an estimated simple payback period of 2.75 years against an estimated turnkey capital cost of 272,384~AED (1,000~AED/kW DC). 

Beyond simple payback, life-cycle discounted cash flow modeling over a 25-year operational lifespan (5.0\% discount rate, 0.5\% annual PV degradation, 1.5\% annual operation and maintenance expense, and 2.0\% tariff escalation) yields a Net Present Value (NPV) of 1,271,167~AED, a discounted payback period of 3.20~years, and a Levelized Cost of Electricity (LCOE) of 0.0943~AED/kWh. Clean solar generation also avoids an estimated 109.34~tCO$_2$e of greenhouse gas emissions annually under the UAE electrical grid emission baseline factor (0.42~kg~CO$_2$e/kWh).
...
\input{tables/tab_sensitivity_setback.tex}
...
\subsubsection{Orientation, Tilt, and Uncertainty Bounds}
...
Due to high direct normal irradiance in the Arabian Gulf, flat installations ($\beta = 0^\circ$) retain 91.2\% of optimal yield. Modules oriented East ($\psi = 90^\circ$) or West ($\psi = 270^\circ$) maintain 85.4\% of optimal yield at standard $15^\circ$ racking. Only arrays oriented toward the northern quadrant ($\psi < 45^\circ$ or $\psi > 315^\circ$) with steep tilts experience severe derating ($>25\%$). 

To assess financial and yield risk under stochastic solar resource and tariff variations, Monte Carlo uncertainty propagation ($N = 1{,}000$ iterations) was evaluated for Building W5. Stochastically sampling irradiance ($\mu=5.50, \sigma=0.25$~PSH), performance ratio ($\mu=0.85, \sigma=0.03$), tariff rates (0.35 to 0.42~AED/kWh), and turnkey capital costs ($1{,}000 \pm 60$~AED/kW) reveals 5th, 50th, and 95th percentile annual yields of 236.52~MWh, 260.58~MWh, and 287.26~MWh, respectively, corresponding to simple payback ranges between 2.30 and 3.17~years. Furthermore, module degradation sweeps across 0.3\%/year to 1.0\%/year confirm 25-year cumulative energy production between 5,783.98~MWh and 6,279.25~MWh. This confirms that in low-latitude, high-irradiance regions, flat-roof ballasted systems remain economically viable across diverse architectural orientations and operational conditions.
...
\begin{enumerate}
    ...
    \item \textbf{Flat-Roof Assumption:} Sizing models assume horizontal rooftops where modules are mounted on ballasted racking. While flat roofs represent the dominant architectural typology for commercial, institutional, and industrial facilities across the Arabian Gulf, this assumption does not capture complex pitched residential roofs with multiple slope facets.
    \item \textbf{Micro-Obstruction and Shading Omission:} Raw vector footprints do not capture rooftop mechanical units (air handling units, chiller pipes, satellite fixtures) or cast-shadow profiles from neighboring high-rise structures.
    \item \textbf{Dynamic Soiling and Thermal Cycling:} Elevated ambient desert temperatures pushing cell temperatures above 65$^\circ$C and dust accumulation affect operational yields. While our empirical derates and Monte Carlo uncertainty sweeps bound bulk performance losses, operational plant design requires site-specific soiling wash cycles and dynamic thermal time-series.
    \item \textbf{Grid Interconnection Constraints:} Pre-feasibility estimates quantify physical rooftop hosting potential, but do not evaluate distribution transformer capacity headroom, reverse power flow limits, or utility grid interconnect requirements.
    \item \textbf{Screening Scope:} The framework is designed strictly for preliminary decision support. It does not replace detailed structural load calculations, electrical single-line engineering, or municipal permitting.
\end{enumerate}
